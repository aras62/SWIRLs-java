...
/**
 * Robot class for multi-threaded implementation using single channel
 * @author Amir Rasouli
 *
 */
public class RobotMultiSingleChannel implements Runnable {
	/**
	 * Constructor 
	 ...
	 */
	public RobotMultiSingleChannel(int dest, int speed,Point loc, char id, 
			Map<Integer,Point> sList,JFrame f, ConcurrentMap<Character,Component> gc, Color col,
			Map<Integer,BlockingQueue<Message>> mList,	Map<Character, Message> tList, BlockingQueue<Boolean> rIsA)
	{
		this.currentLoc = -1;
	...
	/**
	 * Finds closes sensor to the robot
	 * @param sensorsList
	 * @return ID of closest sensor
	 */
	...
	private void sendQuery()
	{
		...
		if (this.distToDest > SwirlController.DELTA )
		{
			...
			try {
				this.msgList.get(this.currentLoc).put (new Message(this.myId,Message.msgType.QUERY,this.destination,this.currentLoc));
	...
	private void driveToNextLocation()
	{
		...
		{	
			...
			this.msgList.get(this.currentLoc).offer (new Message(this.myId,Message.msgType.UPDATE,this.timeElapsed,
					this.currentLoc,this.nextLoc, this.destination));
		}
